...
export default function MobileNumberVerification() {
    ...
    const { remainingSeconds, startTimer, resetTimer, isTimerLoaded } =
        useTimerZustand('verify-mobile-onboarding', 300)
    ...
    const checkNumberVerifiedProgress = async () => {
    ...
    //call when mount
    const isMobileNumberOTPSent = useOnboardingStore(
        (state) => state.isMobileNumberOTPSent
    )
    const setIsMobileNumberOTPSentTrue = useOnboardingStore(
        (state) => state.setIsMobileNumberOTPSentTrue
    )
    ...
    useEffect(() => {
        ...
        const verify = async () => {
            const verified = await checkNumberVerifiedProgress()

            if (verified) return
            if (isMobileNumberOTPSent) return
    ...
    const setIsStep1CompletedTrue = useOnboardingStore(
        (state) => state.setIsStep1CompletedTrue
    )
    ...
    // skip
    const isStep1Completed = useOnboardingStore(
        (state) => state.isStep1Completed
    )
    const setIsStep1CompletedFalse = useOnboardingStore(
        (state) => state.setIsStep1CompletedFalse
    )
    const handleSkip = () => {
        setIsMobileNumberOTPSentTrue()
        if (isStep1Completed !== 'completed' && !numberVerified) {
            setIsStep1CompletedFalse()
        }
        if (numberVerified) {
            setIsStep1CompletedTrue()
        }
            ...
            >
                <div className="bg-white rounded-xl shadow-2xl p-10 w-[90%] md:w-[50%] flex flex-col items-center justify-center">
                    <div className="w-full border-b-2 border-tertiary mb-[20px]">
                        <OnboardingStepper
                            stepNumber={'1'}
                            stepTitle={'SMS Verification'}
                            onPressSkip={handleSkip}
                            isStepCompleted={numberVerified}
                            backDisabled={true}
                        />
                    </div>
                    {loading ? (
                                ...
                                <>
                                    ...
                                    <div className="flex flex-col w-full items-center mb-[20px]">
                                        <div className="text-[16px] 2sm:text-[20px] text-[#121212] mb-2 text-center">
                                            Your mobile number was successfully
                                            verified.
                                        </div>
                                    </div>
                                ...
                                <>
                                    ...
                                    <div className="w-full flex flex-col items-center justify-center">
                                        <Form className="flex flex-col gap-5">
                                            ...
                                            {verifyError && (
                                                <h6 className="w-full text-center text-h6 text-tertiary-700 font-poppins">
                                                    {verifyError}
                                                </h6>
                                            )}
                                        ...
                                        {remainingSeconds === 0 && (
                                            <div className="w-full flex items-center justify-center mt-[30px] mb-[30px]">
                                                ...
                                                <button
                                                    ...
                                                    disabled={
                                                        remainingSeconds > 0
                                                    } // prevent clicking while countdown is running
                                                >
